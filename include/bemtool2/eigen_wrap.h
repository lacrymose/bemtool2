...
template <class eigen_mat_t,class val_type>
  class eigen_mat{
 ...
 public:
  ...
  typedef eigen_mat<eigen_mat_t,val_type>   mat_t;
  typedef vect<val_type>               vect_t;
  typedef val_type                        v_t;

  eigen_mat(){};
  eigen_mat(const int n, const int m): mat(n,m), ipvt(n){};

  val_type& operator()(const int i, const int j){
    return mat(i,j);}

  const val_type& operator()(const int i, const int j) const{
    return mat(i,j);}
...
// //     gmm::clear(m.mat);}
  ...
  //   Eigen::Map<Eigen::VectorXd>(x[0],size(b))
...
typedef eigen_mat< DenseMatrix ,Field > eigen_dense;
