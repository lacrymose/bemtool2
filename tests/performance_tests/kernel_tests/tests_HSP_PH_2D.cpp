...
int main(int argc, char const *argv[]) {
  ...
  modified_bem<P1_1D,P1_1D, TDLP_PH_2D>   M_Kop(kappa,n_,n_,-0.5);
  ...
  HMatrix<modified_bem<P1_1D,P1_1D, TDLP_PH_2D>> MK(M_Kop,nbdof);
  htool::HMatrix<htool::fullACA,Cplx> HW(W,x,tab);
  htool::HMatrix<htool::fullACA,Cplx> HMK(MK,x,tab);
  ...
  for (int l=0;l<1;l++){
    ...
    gN=Ep;
    //fill(gN,Cplx(0.));
    HMK.mvprod_global(gN.data(),f_global.data());
    ...
    ////=== Solve
    const std::vector<std::vector<int>>& MasterClusters=HW.get_MasterClusters();
    int n_local= HW.get_local_size_cluster();
    ...
    htool::HPDDMOperator<htool::fullACA,Cplx> A_HPDDM(HW,P);

    complex<double>* const f = &(f_local[0]);
    complex<double>* sol = &(x_local[0]);
    HPDDM::IterativeMethod::solve(A_HPDDM, f, sol, 1,HW.get_comm());
		...
		MPI_Allgatherv(&(x_local.front()), recvcounts[rankWorld], MPI_DOUBLE_COMPLEX, &(rcv.front()), &(recvcounts.front()), &(displs.front()), MPI_DOUBLE_COMPLEX, HW.get_comm());
    ...
    for(int j=0; j<nbdof; j++){
  	...
  	error2=abs(val/norm);
		double compression = HW.compression();
		int nb_lrmat = HW.get_nlrmat();
		int nb_dmat  = HW.get_ndmat();
		// MPI_Barrier(MPI_COMM_WORLD);
		HW.print_stats();
		if (rankWorld==0){
