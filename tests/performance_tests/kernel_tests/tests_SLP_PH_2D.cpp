...
int main(int argc, char const *argv[]) {
  ...
  htool::HMatrix<htool::fullACA,Cplx> HV(V,x,tab);
  ...
  for (int l=0;l<1;l++){
    ...
    std::vector<Cplx> f_global(nbdof,0),gD(nbdof,0);
    // Boundary condition
    gD=Ep;
    //fill(gD,Cplx(0.));
    HMK.mvprod_global(gD.data(),f_global.data());
    ...
    ////=== Solve
    const std::vector<std::vector<int>>& MasterClusters=HV.get_MasterClusters();
    int n_local= HV.get_local_size_cluster();
    ...
    htool::HPDDMOperator<htool::fullACA,Cplx> A_HPDDM(HV,P);

    complex<double>* const f = &(f_local[0]);
    complex<double>* sol = &(x_local[0]);
    HPDDM::IterativeMethod::solve(A_HPDDM, f, sol, 1,HV.get_comm());
		...
		MPI_Allgatherv(&(x_local.front()), recvcounts[rankWorld], MPI_DOUBLE_COMPLEX, &(rcv.front()), &(recvcounts.front()), &(displs.front()), MPI_DOUBLE_COMPLEX, HV.get_comm());
    ...
    for(int j=0; j<nbdof; j++){
  	...
  	error2=abs(val/norm);
		double compression = HV.compression();
		int nb_lrmat = HV.get_nlrmat();
		int nb_dmat  = HV.get_ndmat();
		// MPI_Barrier(MPI_COMM_WORLD);
		HV.print_stats();
		if (rankWorld==0){
